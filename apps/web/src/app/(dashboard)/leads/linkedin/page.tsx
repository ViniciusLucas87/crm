...
export default function LinkedInLeadsPage() {
  ...
  async function draft(id: number) { setSaving(true); await fetch(`/api/linkedin/opportunities/${id}/draft`, { method: "POST" }); await load(); setSaving(false); }
  async function approve(id: number) {
    const reason = reasons[id]?.trim();
    if (!reason || reason.length < 5) { setError("Record why this is a relevant and respectful connection request."); return; }
    setSaving(true); setError("");
    const response = await fetch(`/api/linkedin/opportunities/${id}/approve-dm`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ human_approved: true, permission_basis: reason }) });
    if (!response.ok) setError("The connection note could not be approved."); await load(); setSaving(false);
  }
  async function contacted(id: number) { setSaving(true); await fetch(`/api/linkedin/opportunities/${id}/mark-contacted`, { method: "POST" }); await load(); setSaving(false); }
  ...
  return <div className="space-y-6">
    ...
    <Card><div className="flex items-center justify-between"><h2 className="font-semibold text-white">Owner queue</h2><span className="text-xs text-slate-500">{items.length} owners</span></div>
      {loading ? <div className="py-12 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-cyan-400" /></div> : items.length === 0 ? <div className="py-12 text-center"><Linkedin className="mx-auto h-8 w-8 text-slate-600" /><p className="mt-3 text-sm text-white">No LinkedIn owners researched yet</p></div> : <div className="mt-4 space-y-3">{items.map(item => <div key={item.id} className="rounded-xl border border-white/10 bg-slate-950/40 p-4">
        ...
        {!item.public_reply_draft ? <div className="mt-3 flex justify-end"><Button variant="secondary" size="sm" onClick={() => void draft(item.id)}><Sparkles className="mr-1 h-3.5 w-3.5" />Prepare connection note</Button></div> : !item.human_approved_at ? <div className="mt-3 rounded-lg border border-amber-400/15 bg-amber-400/5 p-3"><p className="text-xs text-slate-400">Review the profile. Record why this specific contact is relevant before sending anything.</p><textarea className="mt-3 min-h-16 w-full rounded-lg border border-white/10 bg-slate-900 px-3 py-2 text-sm text-white" placeholder="Why is this a relevant connection?" value={reasons[item.id] || ""} onChange={e => setReasons(current => ({ ...current, [item.id]: e.target.value }))} /><div className="mt-3 flex justify-end"><Button variant="secondary" size="sm" onClick={() => void approve(item.id)}>Approve note</Button></div></div> : <div className="mt-3 rounded-lg border border-emerald-400/15 bg-emerald-400/5 p-3"><div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-400" /><p className="text-xs font-semibold text-emerald-200">Approved for personal sending</p></div>{item.dm_draft && <p className="mt-3 text-sm text-slate-300">{item.dm_draft}</p>}<div className="mt-3 flex justify-end">{item.contacted_at ? <Badge variant="success">Contact recorded</Badge> : <Button variant="primary" size="sm" onClick={() => void contacted(item.id)}>I sent this</Button>}</div></div>}
      </div>)}</div>}
    </Card>
  </div>;
}
