...
async def telnyx_webhook(request: Request, session: Session = Depends(get_db_session)):
    """Receive Telnyx call state webhooks. Idempotent."""
    ...
    if not call:
        # Inbound call — create new Call record
        caller_number = payload.get("from", "")
        destination = payload.get("to", "")
        entities = lifecycle.resolve_entities(caller_number)
    ...
    logger.info("Webhook processed: event=%s call=%s state=%s", event_type, call.public_uuid, call.status)
